...
async def submit_question(
    ...
    try:
        ...
        # Save to Firestore
        def _save_question():
            db.collection("qa").document(question_id).set(question_data)
...
async def get_answer(question_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Save to Firestore
        def _update_answer():
            db.collection("qa").document(question_id).update(update_data)
        ...
        if question_id:
            def _mark_failed():
                db.collection("qa").document(question_id).update({
                    "status": "failed",
                    "error": str(e)
                })
    ...
    try:
        def _get_history():
            ...
            if question_id:
                ...
                doc = db.collection("qa").document(question_id).get()
