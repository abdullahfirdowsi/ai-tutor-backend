...
async def create_qa_session(
    ...
    Args:
        ...
        lesson_id: Optional ID of a related lesson
    ...
    try:
        ...
        # Save to Firestore
        def _save_session():
            get_db().collection("qa_sessions").document(session_id).set(session_data)
            return session_data
...
async def get_qa_sessions(
    ...
    try:
        def _get_sessions():
            # Start with a base query
            query = get_db().collection("qa_sessions").where("user_id", "==", user_id)
            
            # Only include active sessions if specified
...
async def get_qa_session(
    ...
    Returns:
        Optional[Dict]: Session data if found and authorized
    """
    try:
        def _get_session():
            doc = get_db().collection("qa_sessions").document(session_id).get()
            if not doc.exists:
                return None
...
async def update_qa_session(
    ...
    try:
        ...
        def _update_session():
            get_db().collection("qa_sessions").document(session_id).update(update_dict)
            return {**session, **update_dict}
...
async def delete_qa_session(
    ...
    try:
        ...
        def _delete_session():
            get_db().collection("qa_sessions").document(session_id).delete()
            return True
